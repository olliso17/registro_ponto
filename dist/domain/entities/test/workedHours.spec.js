...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`
  ...
  function createClock(start, loopLimit) {
    ...
    function doTick(tickValue, isAsync2, resolve3, reject) {
...
// src/domain/entities/workedHours.ts
...
// src/domain/entities/workedHours.ts
var WorkedHoursSchema = z.object({
  id: z.string(),
  hours_worked: z.string().regex(/^\d{1,2} \d{1,2}:\d{1,2} \d{1,2}:\d{1,2}$/)
});
var WorkedHours = class {
  ...
  static validateInstance(instance) {
    try {
      ...
      return ["Erro ao validar os dados da inst\xE2ncia de WorkedHours."];
    }
  }
};

// src/domain/entities/test/workedHours.spec.ts
test("Teste de valida\xE7\xE3o da entidade WorkedHours", (t) => {
  const validInstance = new WorkedHours();
  assert.ok(WorkedHours.validateInstance(validInstance), "Inst\xE2ncia v\xE1lida deve passar na valida\xE7\xE3o");
  const invalidInstance = new WorkedHours();
  invalidInstance["_hours_worked"] = "25 12:12 12:12";
  assert.throws(() => WorkedHours.validateInstance(invalidInstance), "Erro de valida\xE7\xE3o dos dados da inst\xE2ncia de WorkedHours");
});
