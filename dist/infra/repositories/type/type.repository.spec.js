  ...
  async getAllType() {
    try {
      ...
      typeData.map((typeData2) => {
        const type2 = new TypeEntity(typeData2);
        typeAll.push(type2);
      });
      return typeAll;
    } catch (error) {
...
// src/infra/repositories/type/type.repository.spec.ts
describe("Type Repository", () => {
  const typeReppository = new TypeRepository();
  test("not create employee", async () => {
    const types = await typeReppository.getAllType();
    await globalExpect(types).toHaveLength(types.length);
  });
});
